...
def parse(filename, isCsv, hideHeaders, hideProfiles, hideActors, dpsOnly):
    ...
    if(hideHeaders):
        ...
        if not dpsOnly:
            ret += separator+'agi'+separator+'haste'+separator+'crit'+separator+'mastery'+separator+'vers\n'
    with open(filename, "r") as f:
        s = f.read()
        sim = json.loads(s)
        results = sim['sim']['players']
        for player in sorted(results, key=lambda k: k['name']):
            if dpsOnly or 'Agi' in player['scale_factors']:
                if(not hideProfiles):
                ...
                if not dpsOnly:
                    ...
                    ret+= '{0:.{1}f}'.format(weights['Agi'],2) + separator
...
def parseProfileSets(filename, isCsv, hideHeaders, hideProfiles, hideActors, dpsOnly):
    ...
    with open(filename, "r") as f:
        ...
        for profile in sorted(results, key=lambda k: k['name']):
            if dpsOnly or 'Agi' in profile['scale_factors']:
                if(not hideProfiles):
                    ret+= path.splitext(filename)[0]+separator
                if(not hideActors):
